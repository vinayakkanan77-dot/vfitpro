...
async function sendStitchedEmail({ customer, shop, log }) {
  const imageUrls = Array.isArray(customer.stitchedImageUrls) ? customer.stitchedImageUrls : [];

  const { subject, htmlContent } = buildStitchedEmail({ customer, shop, imageUrls });
  ...
  log?.info('Firestore update: customer audit trail', { customerId: customer.id, messageId });
  await db
    .collection(COLLECTIONS.CUSTOMERS)
    .doc(customer.id)
    .set(
      { lastStitchedEmailSentAt: admin.firestore.FieldValue.serverTimestamp(), lastStitchedEmailMessageId: messageId },
      { merge: true }
    );
